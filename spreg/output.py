...
def output_start(reg):
    ...
    strSummary += "------------------\n"
    reg.output = reg.output.assign(
        coefficients=[None] * len(reg.output),
        std_err=[None] * len(reg.output),
        zt_stat=[None] * len(reg.output),
        prob=[None] * len(reg.output),
    )
    return strSummary
...
def out_part_top(strSummary, reg, m):
    ...
    if m == None:
        ...
        _reg = reg.multi[
            m
        ]  # _reg = local object with equation specific regression results
...
def out_part_middle(strSummary, reg, robust, m=None, latex=False):
    ...
    try:
        coefs = pd.concat(
            [coefs, pd.DataFrame(_reg.z_stat, columns=["zt_stat", "prob"])], axis=1
        )
    except AttributeError:
        coefs = pd.concat(
            [coefs, pd.DataFrame(_reg.t_stat, columns=["zt_stat", "prob"])], axis=1
        )
    ...
    if latex:
        ...
        df_2 = df_2.set_axis(
            [
                ...
                "Coefficient",
                "Std.Error",
                _reg.__summary["summary_zt"] + "-Statistic",
                "Prob.",
            ],
            axis="columns",
            copy=False,
        ...
        df_2["Variable"] = (
            df_2["Variable"].str.replace("_", "\_").str.replace("%", "\%")
        )
        ...
        for row in m_output.iloc[
            np.lexsort((m_output.index, m_output["regime"]))
        ].itertuples():
            try:
                ...
                TypeError
            ):  # special case for models that do not have inference on the lambda term
                strSummary += "%20s    %12.5f    \n" % (row.var_names, row.coefficients)
        strSummary += "------------------------------------------------------------------------------------\n"
    ...
    try:  # Adding info on regimes if they are present
        ...
        strSummary += _summary_chow(
            _reg
        )  # If local regimes present, compute Chow test.
    except:
        pass
...
def _summary_chow(reg):
    ...
    reg.output_chow = pd.concat(
        [reg.output_chow, pd.DataFrame(regi, columns=["value", "prob"])], axis=1
    )
    reg.output_chow = pd.concat(
        [
            reg.output_chow,
...
def _spat_diag_out(reg, w, type, moran=False, ml=False):
    ...
    if type == "yend":
        ...
        if any(reg.output["var_type"] == "rho"):
            ...
            if reg.slx_lags == 1 and not any(reg.output["var_type"] == "yend"):
                if not hasattr(reg, "slx_vars") or not isinstance(reg.slx_vars, list):
                    wx_indices = reg.output[
                        (reg.output["var_type"] == "wx")
                        & (reg.output["regime"] != "_Global")
                    ].index
                    x_indices = []
                    for m in reg.output["regime"].unique():
                        x_indices.extend(
                            reg.output[
                                (reg.output["regime"] == m)
                                & (reg.output["var_type"] == "x")
                            ].index[1:]
                        )
                    vm_indices = (
                        x_indices
                        + wx_indices.tolist()
                        + reg.output[reg.output["var_type"] == "rho"].index.tolist()
                    )
                    cft, cft_p = diagnostics_sp.comfac_test(
                        reg.rho,
                        reg.betas[x_indices],
        ...
        lm_tests = diagnostics_sp.LMtests(
            reg, w, tests=["lme", "lml", "rlme", "rlml", "sarma"]
        )
        if reg.slx_lags == 0:
            try:
                lm_tests2 = diagnostics_sp.LMtests(
                    reg,
                    w,
                    tests=["lmwx", "lmspdurbin", "rlmdurlag", "rlmwx", "lmslxerr"],
                )
...
def _summary_impacts(reg, w, spat_impacts, slx_lags=0, slx_vars="All", regimes=False):
    ...
    variables = reg.output.query(
        "var_type == 'x' and index != 0"
    )  # excludes constant and endogenous variables
    ...
    for i in spat_impacts:
        spmult = _spmultiplier(
            w, reg.rho, method=i
        )  # computes the multipliers, slx_lags not needed

        strSummary += spmult["warn"]
        btot, bdir, bind = _sp_effects(
            reg, variables, spmult, slx_lags, slx_vars
        )  # computes the impacts, needs slx_lags
        sp_multipliers[spmult["method"]] = (
            spmult["adi"],
            spmult["aii"].item(),
            spmult["ati"].item(),
        )
        ...
        for i in range(len(variables)):
            strSummary += "%20s   %12.4f    %12.4f    %12.4f\n" % (
                variables["var_names"][variables_index[i]],
                bdir[i][0],
                bind[i][0],
                btot[i][0],
            )
