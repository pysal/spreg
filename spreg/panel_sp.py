...
class BaseGM_KKP(RegressionPropsY):
    ...
    full_w    : boolean
                if True: use different weights for each of 6 moment conditions
                if False: use only 2 sets of weights for the first 3 and the
                last 3 moment conditions
    ...
    def __init__(self, y, x, w, full_w=False):
        ...
        G = np.vstack(
            (np.hstack((moments[0], np.zeros((3, 1)))), moments_b[0]))
        moments6 = [G, np.vstack((moments[1], moments_b[1]))]
        lambda2, sig_vb, sig_1b = optim_moments(
            moments6, vcX=Xi.toarray(), all_par=True, start=[
                lambda1, sig_v, sig_1])
        # 2a. reg -->\hat{betas}
...
class GM_KKP(BaseGM_KKP, REGI.Regimes_Frame):
    ...
    we set full_w to False (the default), indicating that we will use
    only 2 sets of moments weights for the first 3 and the last 3 moment conditions.

    >>> reg = GM_KKP(y,x,w,full_w=False,name_y=name_y, name_x=name_x)
    ...
    def __init__(self, y, x, w,
                 method="random", lag=False, error="kkp", full_w=False,
        ...
        if regimes is not None:
            ...
            x_constant, self.name_x = REGI.Regimes_Frame.__init__(
                self, x_constant, regimes_l, constant_regi=False, cols2regi='all', names=self.name_x)

        BaseGM_KKP.__init__(self, bigy, x_constant, w, full_w=full_w)
    ...
    def _set_regimes(self, w, n_rows):  # Must add case for regime_err_sep = True
        ...
        if len(self.regimes) == w.n:
            ...
            raise Exception(
                "The lenght of 'regimes' must be either equal to n or n*t.")
        return regimes_l
...
def _moments_kkp(ws, u, i, trace_w2=None):
    ...
    if i == 0:
        G = np.array([[G11, G12, N * (T - 1)**(1 - i)],
                      [G21, G22, G23], [G31, G32, 0]]) / (N * (T - 1)**(1 - i))
    else:
        G = np.array([[G11, G12, 0, N * (T - 1)**(1 - i)], [G21,
                                                            G22, 0, G23], [G31, G32, 0, 0]]) / (N * (T - 1)**(1 - i))
