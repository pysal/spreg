...
(lagged X and spatial error) models. These are syntactic sugar
for alternative uses of the Lag and Error classes.
"""
...
class DurbinLag(Lag):
    ...
    def fit(self, X, y, yend=None, q=None, w_lags=1, lag_q=True, method="gm", epsilon=1e-7):
        ...
        super().fit(X, y, yend, q, w_lags, lag_q, method, epsilon)
...
class DurbinError(Error):
    ...
    def fit(self, X, y, yend=None, q=None, method="gm", epsilon=1e-7):
        ...
        X = np.hstack((X, self.w.full()[0] @ X))
        super().fit(X, y, yend, q, method, epsilon)
