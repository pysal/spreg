...
def Panel_FE_Error(reg, w, vm, regimes=False):
    reg.__summary = {}
    # compute diagnostics and organize summary output
    beta_diag(reg, robust=None)
    reg.__summary["summary_r2"] += "%-20s:%12.3f                %-22s:%12.3f\n" % (
    ...
    for eq in reg.name_bigy.keys():
        ...
        for k in range(regK):
            ...
            try:
                ...
                fixed_rho = 0
                if k == range(regK)[-1] and (
                    len(reg.name_bigyend[eq]) / regK
                ) * regK < len(reg.name_bigyend[eq]):
                    fixed_rho += 1
                for j in range(len(reg.name_bigyend[eq]))[
                    int(k * regkend) : int((k + 1) * regkend + fixed_rho)
                ]:
                    strSummary += "%20s    %12.7f    %12.7f    %12.7f    %12.7f\n" % (
                        reg.name_bigyend[eq][j],
                        betas[eq][h + j][0],
                        inf[eq][h + j][0],
                        inf[eq][h + j][1],
                        inf[eq][h + j][2],
...
def summary_coefs_slopes(reg):
    ...
    strSummary += "------------------------------------------------------------------------------------\n"
    strSummary += (
        "            Variable           Slope       Std.Error     %1s-Statistic     Probability\n"
        % (reg.__summary["summary_zt"])
    )
    strSummary += "------------------------------------------------------------------------------------\n"
...
def summary_diag_sur(
    reg, nonspat_diag, spat_diag, tsls, lambd, rho=False, ml=True, regimes=False
):
    if not ml:
        try:
            if reg.joinrho != None:
