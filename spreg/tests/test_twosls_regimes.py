...
class TestTSLS(unittest.TestCase):
    ...
    def test_basic(self):
        reg = TSLS_Regimes(
            self.y, self.x, self.yd, self.q, self.regimes, regime_err_sep=False
        )
        betas = np.array(
            [
                [80.23408166],
        ...
        h_0 = np.array([[0.0, 0.0, 1.0, 19.531, 0.0, 5.03]])
        np.testing.assert_allclose(reg.h[0] * np.eye(6), h_0, RTOL)
        hth = np.array(
            [
                [25.0, 416.378999, 0.0, 0.0, 76.03, 0.0],
                [416.378999, 7831.05477839, 0.0, 0.0, 1418.65422625, 0.0],
                [0.0, 0.0, 24.0, 287.993, 0.0, 63.72],
                [0.0, 0.0, 287.993, 3855.61860282, 0.0, 827.47378],
                [76.03, 1418.65422625, 0.0, 0.0, 291.9749, 0.0],
                [0.0, 0.0, 63.72, 827.47378, 0.0, 206.6102],
            ]
        )
        np.testing.assert_allclose(reg.hth, hth, RTOL)
        hthi = np.array(
            [
                [0.3507855, -0.0175615, 0.0, 0.0, -0.00601601, 0.0],
                [-0.0175615, 0.00194521, -0.0, -0.0, -0.00487844, -0.0],
                [0.0, 0.0, 0.42327489, -0.02563036, 0.0, -0.02789128],
                [-0.0, -0.0, -0.02563036, 0.00339841, -0.0, -0.00570605],
                [-0.00601601, -0.00487844, 0.0, 0.0, 0.02869498, 0.0],
                [0.0, 0.0, -0.02789128, -0.00570605, 0.0, 0.03629464],
            ]
        )
        ...
        pfora1a2 = np.array(
            [
                [17.80208995, -0.46997739, 0.0, 0.0, -0.21344994, 0.0],
                [-0.36293902, 0.41200496, 0.0, 0.0, -0.17308863, 0.0],
                [0.0, 0.0, 23.8584271, -0.96035493, 0.0, -0.26149141],
                [0.0, 0.0, -0.61800983, 0.2269828, 0.0, -0.05349643],
                [-3.22151864, -2.10181214, 0.0, 0.0, 1.01810757, 0.0],
                [0.0, 0.0, -5.42403871, -0.6641704, 0.0, 0.34027606],
            ]
        )
        np.testing.assert_allclose(reg.pfora1a2, pfora1a2, RTOL)
        predy_5 = np.array(
            [[-9.85078372], [36.75098196], [57.34266859], [42.89851907], [58.9840913]]
        )
        ...
        varb = np.array(
            [
                [0.50015831, 0.07969376, 0.0, 0.0, -0.04760541, 0.0],
                [0.07969376, 0.06523527, 0.0, 0.0, -0.03105915, 0.0],
                [0.0, 0.0, 0.73944792, 0.01132445, 0.0, -0.02117969],
                [0.0, 0.0, 0.01132445, 0.00756336, 0.0, -0.00259344],
                [-0.04760541, -0.03105915, -0.0, -0.0, 0.0150449, -0.0],
                [-0.0, -0.0, -0.02117969, -0.00259344, -0.0, 0.0013287],
            ]
        )
        np.testing.assert_allclose(reg.varb, varb, RTOL)
        vm = np.array(
            [
                [495.16048523, 78.89742341, 0.0, 0.0, -47.12971066, 0.0],
                [78.89742341, 64.58341083, 0.0, 0.0, -30.74878934, 0.0],
                [0.0, 0.0, 732.05899155, 11.21128921, 0.0, -20.96804956],
                [0.0, 0.0, 11.21128921, 7.48778398, 0.0, -2.56752553],
                [-47.12971066, -30.74878934, 0.0, 0.0, 14.89456384, 0.0],
                [0.0, 0.0, -20.96804956, -2.56752553, 0.0, 1.3154267],
            ]
        )
        np.testing.assert_allclose(reg.vm, vm, RTOL)
        x_0 = np.array([[0.0, 0.0, 1.0, 19.531]])
        ...
        yend_3 = np.array([[0.0, 80.467003], [0.0, 44.567001], [0.0, 26.35]])
        np.testing.assert_allclose(reg.yend[0:3] * np.eye(2), yend_3, RTOL)
        z_0 = np.array([[0.0, 0.0, 1.0, 19.531, 0.0, 80.467003]])
    ...
    def test_regi_err(self):
        ...
        model = TSLS_Regimes(
            y, x1, regimes=regi, q=q, yend=x2, regime_err_sep=True, sig2n_k=False
        )
        model1 = TSLS(
        ...
        vm = np.array([495.16048523, 78.89742341, 0.0, 0.0, -47.12971066, 0.0])
