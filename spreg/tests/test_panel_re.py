...
class Test_Panel_RE_Lag(unittest.TestCase):
    ...
    def test_Panel(self):
        reg = Panel_RE_Lag(self.y, self.x, w=self.w,
                           name_y=self.y_name, name_x=self.x_names,
                           name_ds=self.ds_name)
        betas = np.array([[4.44421994],
                          [2.52821717],
                          [2.24768846],
                          [0.25846846],
                          [0.68426639]])
        ...
        vm = np.array([0.08734092, 0.05232857, 0.05814063, 0.00164801,
                       0.00086908])
        ...
        std_err = np.array([0.29553498, 0.22875438, 0.24112368, 0.04059565,
                            0.02948021])
...
class Test_Panel_RE_Error(unittest.TestCase):
    ...
    def test_Panel(self):
        reg = Panel_RE_Error(self.y, self.x, w=self.w,
                             name_y=self.y_name, name_x=self.x_names,
                             name_ds=self.ds_name)
        betas = np.array([[5.87893756],
                          [3.23269025],
                          [2.62996804],
                          [0.34042682],
                          [4.9782446]])
        ...
        np.testing.assert_allclose(reg.predy[0], predy, RTOL)
        vm = np.array([0.05163595, 0.05453637, 0.06134783, 0.00025012,
                       0.0030366])
        np.testing.assert_allclose(reg.vm.diagonal(), vm, RTOL)
