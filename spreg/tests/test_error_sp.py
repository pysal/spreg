...
class TestGMMError(unittest.TestCase):
    ...
    def test_model(self):
       ...
       [-1.04655994e-02, -2.75387247e-04,  5.42396309e-05,
        ...
        reg = SP.GMM_Error(self.db[["HOVAL"]], self.db[["INC"]],
                            yend=self.db[["CRIME"]], 
                            q=self.db[["DISCBD"]], 
                            w=self.w, slx_lags=1) 
        betas = np.array([ 
        ...
        np.testing.assert_allclose(reg.betas,betas,RTOL)
        vm = np.array([
            [912.63997789, -19.62464020, -7.03524948, -13.72433338, 1.84512875],
            [-19.62464020, 0.52955014, -0.00617855, 0.31294813, -0.04530781],
            [-7.03524948, -0.00617855, 0.37552517, 0.05701885, 0.00382282],
            [-13.72433338, 0.31294813, 0.05701885, 0.22015356, -0.03136501],
            [1.84512875, -0.04530781, 0.00382282, -0.03136501, 0.03001728]])
        np.testing.assert_allclose(reg.vm,vm,RTOL)

        reg = SP.GMM_Error(self.db[["HOVAL"]], self.db[["INC", "CRIME"]],
                                   slx_vars="All", w=self.w, slx_lags=1) 
        betas = np.array([
            [29.38238574],
            [ 0.82921502],
            [-0.57499819],
        ...
        np.testing.assert_allclose(reg.betas,betas,RTOL)
        vm = np.array([
            [5.97495766e+02, -5.55463269e+00, -6.81492201e-01, -1.53802421e+01, -7.21116007e+00, 0.00000000e+00],
            [-5.55463269e+00, 1.34368489e-01, 2.10846256e-02, 3.83654841e-02, 5.54102390e-02, 0.00000000e+00],
            [-6.81492201e-01, 2.10846256e-02, 2.56742519e-02, 2.40939941e-03, -1.84082192e-02, 0.00000000e+00],
            [-1.53802421e+01, 3.83654841e-02, 2.40939941e-03, 6.69703341e-01, 1.58284990e-01, 0.00000000e+00],
            [-7.21116007e+00, 5.54102390e-02, -1.84082192e-02, 1.58284990e-01, 1.30057684e-01, 0.00000000e+00],
            [0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 2.67800026e-02]
            ])
        np.testing.assert_allclose(reg.vm,vm,RTOL)

        reg = SP.GMM_Error(self.db[["HOVAL"]], self.db[["INC", "CRIME"]],
                       slx_vars=[True, True], w=self.w, slx_lags=1) 
        betas = np.array([
            [29.38238574],
            [ 0.82921502],
            [-0.57499819],
