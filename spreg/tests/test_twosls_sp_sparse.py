...
class TestBaseGMLag(unittest.TestCase):
    ...
    def test___init__(self):
        ...
        reg = BaseGM_Lag(self.y, self.X, w=self.w, w_lags=2)
        betas = np.array(
    ...
    def test_init_white_(self):
        ...
        self.X = np.hstack((np.ones(self.y.shape), self.X))
        self.X = SP.csr_matrix(self.X)
        base_gm_lag = BaseGM_Lag(self.y, self.X, w=self.w, w_lags=2, robust="white")
        tbetas = np.array(
            [[4.53017056e01], [6.20888617e-01], [-4.80723451e-01], [2.83622122e-02]]
        )
    ...
    def test_init_hac_(self):
        ...
        base_gm_lag = BaseGM_Lag(
            self.y, self.X, w=self.w, w_lags=2, robust="hac", gwk=gwk
        )
        tbetas = np.array(
            [[4.53017056e01], [6.20888617e-01], [-4.80723451e-01], [2.83622122e-02]]
        )
    ...
    def test_init_discbd(self):
        ...
        self.X = np.hstack((np.ones(self.y.shape), self.X))
        self.X = SP.csr_matrix(self.X)
        reg = BaseGM_Lag(self.y, self.X, yend=yd, q=q, w=self.w, w_lags=2)
    ...
    def test_n_k(self):
        ...
        self.X = np.hstack((np.ones(self.y.shape), self.X))
        self.X = SP.csr_matrix(self.X)
        reg = BaseGM_Lag(self.y, self.X, w=self.w, w_lags=2, sig2n_k=True)
        betas = np.array(
            [[4.53017056e01], [6.20888617e-01], [-4.80723451e-01], [2.83622122e-02]]
        )
        np.testing.assert_allclose(reg.betas, betas, RTOL)
    ...
    def test_lag_q(self):
        ...
        self.X = np.hstack((np.ones(self.y.shape), self.X))
        self.X = SP.csr_matrix(self.X)
        reg = BaseGM_Lag(self.y, self.X, yend=yd, q=q, w=self.w, w_lags=2, lag_q=False)
...
class TestGMLag(unittest.TestCase):
    ...
    def test___init__(self):
        ...
        np.testing.assert_allclose(reg.betas, betas, RTOL)
        e_5 = np.array(
            [[29.28976367], [-6.07439501], [-15.30080685], [-0.41773375], [-5.67197968]]
        )
        ...
        np.testing.assert_allclose(reg.predy[0:5], predy_5, RTOL)
        predy_e_5 = np.array(
            [[51.17723933], [50.64139601], [41.65080685], [33.61773475], [28.89697968]]
        )
    ...
    def test_spatial(self):
        ...
        betas = np.array(
            [[5.46344924e01], [4.13301682e-01], [-5.92637442e-01], [-7.40490883e-03]]
        )
        np.testing.assert_allclose(reg.betas, betas, RTOL)
        vm = np.array(
            [
                [4.45202654e02, -1.50290275e01, -6.36557072e00, -5.71403440e-03],
                [-1.50290275e01, 5.93124683e-01, 2.19169508e-01, -6.70675916e-03],
                [-6.36557072e00, 2.19169508e-01, 1.06577542e-01, -2.96533875e-03],
