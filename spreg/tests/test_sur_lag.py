...
class Test_SURlagIV(unittest.TestCase):
    def setUp(self):
        nat = load_example('NCOVR')
        self.db = libpysal.io.open(nat.get_path('NAT.dbf'), 'r')
        self.w = libpysal.weights.Queen.from_shapefile(nat.get_path("NAT.shp"))
        self.w.transform = 'r'

        self.dbs = gpd.read_file(nat.get_path("NAT.shp"))

    def test_3SLS(self): #2 equations, same K in each
        y_var0 = ['HR80','HR90']
        x_var0 = [['PS80','UE80'],['PS90','UE90']]
        reg = SURlagIV(y_var0,x_var0,w=self.w,df=self.dbs,\
